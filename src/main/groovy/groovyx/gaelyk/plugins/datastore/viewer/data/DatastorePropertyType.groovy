...
enum DatastorePropertyType {
    ...
    BYTE_SHORT("Byte, short", [ShortBlob.class], { _this, value -> value.toString() }, { _this, value -> value }),
    BYTE_LONG("Byte, long", [Blob.class], { _this, value -> value.toString() }, { _this, value -> value }),
    CATEGORY("Category", [Category.class], { _this, value -> value.category }, { _this, value -> value }),
    ...
    FLOAT("Floating Point Number", [Float.class, Double.class], { _this, value -> value.toString() }, { _this, value -> value }),
    GEO_POINT("Geographical Point", [GeoPt.class], { _this, value -> value.toString() }, { _this, value -> value }),
    GOOGLE_ACCOUNT_USER("Google Account User", [User.class], { _this, value -> value.toString() }, { _this, value -> value }),
    INTEGER("Integer", [Short.class, Integer.class, Long.class], { _this, value -> value.toString() }, { _this, value -> value }),
    ...
    MESSAGING_HANDLE("Messaging Handle", [IMHandle.class], { _this, value -> value.address }, { _this, value -> value }),
    ...
    final String label
    final List javaTypes
    final Closure formatValue
    final Closure parseValue

    DatastorePropertyType(String label, List javaTypes, Closure formatValueClosure, Closure parseValueClosure) {
        this.label = label
    ...
    static DatastorePropertyType getPropertyTypeForJavaType(Class klass) {
        for(DatastorePropertyType propertyType : values()) {
            if(propertyType.javaTypes.contains(klass)) {
                return propertyType
            }
        }
